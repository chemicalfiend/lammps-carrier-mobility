...
def fn():
    with gsd.hoomd.open(name="trajectory.gsd", mode='rb') as f:
        snap = f[-1]
...
if __name__ == '__main__':
    master_list = fn()
    system = System("trajectory.gsd", "bbl", frame=-1, scale=3.5636, conversion_dict=bbl_dict)
    system.add_chromophores(master_list, "acceptor", chromophore_kwargs={"reorganization_energy" : 0.324})
    system.compute_energies()
    system.set_energies()
    lifetime = [1e-10, 1e-9]
    temp = 300
    system.run_kmc(lifetime, temp, n_elec=1)
